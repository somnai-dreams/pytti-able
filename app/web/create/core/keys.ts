...
// (frame-prev/next only act while the lightbox is open); the mapping itself is
// scope-free. No other bindings exist.
//
...
//          | 'frame-prev' | 'frame-next' | 'none'
//   keyIntent(facts) -> Intent
// @/cs

export type KeyFacts = { key: string; meta: boolean; ctrl: boolean; inInput: boolean }

export type Intent = 'submit' | 'rerun-last' | 'dismiss' | 'focus-prompt' | 'frame-prev' | 'frame-next' | 'none'
