...
describe('keyIntent (the whole map, spec §9.2)', () => {
  ...
  test('horizontal arrows scrub frames only outside inputs', () => {
  ...
  test('vertical arrows page jobs only outside inputs', () => {
    expect(keyIntent({ ...base, key: 'ArrowUp' })).toBe('job-prev')
    expect(keyIntent({ ...base, key: 'ArrowDown' })).toBe('job-next')
    expect(keyIntent({ ...base, key: 'ArrowDown', inInput: true })).toBe('none')
  })
